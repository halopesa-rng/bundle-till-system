...
app.get('/api/catalog',async(req,res)=>{const r=await query('SELECT id,name,price,network,provider_code AS "providerCode" FROM bundles WHERE active=true ORDER BY price');res.json({till:process.env.TILL_NUMBER||'1677484',currency:process.env.CURRENCY||'KES',bundles:r.rows});});
...
async function approveOrder(orderId, approvedBy){
  ...
  try{
    ...
    if(o.payment_status!=='SUCCESS'){await client.query('ROLLBACK');return {error:'Payment is not confirmed'};}
...
app.post('/api/telegram/webhook/:secret',async(req,res)=>{
  ...
  try{
    ...
    if(action==='reject'){
      await query(`UPDATE orders SET delivery_status='REJECTED',failure_reason='Rejected by admin via Telegram' WHERE id=$1 AND payment_status='SUCCESS'`,[orderId]);
