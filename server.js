...
async function initDb(){
  ...
  const desired=[
    ['250 MB Weekly',20,'Safaricom','B250'],
    ['500 MB Weekly',70,'Safaricom','B500'],
    ['1 GB Weekly',100,'Safaricom','B1000'],
    ['2 GB Monthly',200,'Safaricom','B2000'],
    ['5 GB Monthly',500,'Safaricom','B5000'],
    ['10 GB Monthly',1000,'Safaricom','B10000']
  ];
  // Keep the catalog exactly aligned with the requested bundles.
  for(const [name,price,network,provider_code] of desired){
    const existing=(await query('SELECT id FROM bundles WHERE provider_code=$1 LIMIT 1',[provider_code])).rows[0];
    if(existing){
      await query('UPDATE bundles SET name=$1,price=$2,network=$3,active=true,updated_at=NOW() WHERE id=$4',[name,price,network,existing.id]);
    }else{
      await query('INSERT INTO bundles(id,name,price,network,provider_code) VALUES($1,$2,$3,$4,$5)',[uuid(),name,price,network,provider_code]);
    }
  }
  await query(`UPDATE bundles SET active=false WHERE provider_code NOT IN ('B250','B500','B1000','B2000','B5000','B10000')`);
}
