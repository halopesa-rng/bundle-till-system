...
app.post('/api/payments/stkpush',async(req,res)=>{
  try{
    ...
    const o=(await query('SELECT * FROM orders WHERE order_reference=$1',[orderReference])).rows[0];
    if(!o) return res.status(404).json({error:'Order not found'});
    if(o.payment_status==='SUCCESS') return res.json({success:true,message:'Payment already confirmed',orderReference:o.order_reference});
    const phone=normalizePhone(o.phone);
    const timestamp=new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
    const base=process.env.MPESA_BASE_URL||'https://sandbox.safaricom.co.ke';
    const token=await mpesaToken();
    const callback=process.env.MPESA_STK_CALLBACK_URL || `${process.env.APP_BASE_URL||''}/api/mpesa/stk/callback`;
    if(!callback.startsWith('http')) return res.status(500).json({error:'APP_BASE_URL or MPESA_STK_CALLBACK_URL is not configured'});
    const payload={
      BusinessShortCode:process.env.MPESA_SHORTCODE,
      Password:stkPassword(timestamp),
      Timestamp:timestamp,
      TransactionType:process.env.MPESA_TRANSACTION_TYPE||'CustomerPayBillOnline',
      Amount:Math.round(Number(o.amount)),
      PartyA:phone,
      PartyB:process.env.MPESA_SHORTCODE,
      PhoneNumber:phone,
      CallBackURL:callback,
      AccountReference:o.order_reference,
      TransactionDesc:`Bundle ${o.bundle_name}`.slice(0,100)
    };
    if(!payload.BusinessShortCode||!process.env.MPESA_PASSKEY) return res.status(500).json({error:'M-PESA shortcode/passkey is not configured'});
    ...
    const d=await r.json();
    if(!r.ok || d.ResponseCode!=='0') return res.status(502).json({error:d.errorMessage||d.ResponseDescription||'M-PESA STK Push failed'});
    await query('UPDATE orders SET checkout_request_id=$1 WHERE id=$2',[d.CheckoutRequestID,o.id]);
    await audit('Customer',`STK Push requested for ${o.order_reference}`,'SUCCESS',{checkoutRequestId:d.CheckoutRequestID});
    res.json({success:true,message:'M-PESA prompt sent to the phone',orderReference:o.order_reference,checkoutRequestId:d.CheckoutRequestID});
  }catch(e){console.error('stkpush',e);res.status(500).json({error:e.message||'Unable to start M-PESA payment'});}
});
...
app.post('/api/mpesa/stk/callback',async(req,res)=>{
  try{
    ...
    const o=checkout ? (await query('SELECT * FROM orders WHERE checkout_request_id=$1',[checkout])).rows[0] : null;
    if(Number(cb.ResultCode)===0 && o){
      const items=cb.CallbackMetadata?.Item||[];
      const val=n=>items.find(x=>String(x.Name).toLowerCase()===n.toLowerCase())?.Value;
      const amount=Number(val('Amount')||o.amount);
      const phone=normalizePhone(val('PhoneNumber')||o.phone);
      const receipt=String(val('MpesaReceiptNumber')||checkout);
      const result=await processPayment({amount,phone,receipt,account:o.order_reference,body:req.body});
      if(result.orderId) deliverBundle(result.orderId).catch(e=>console.error('delivery',e));
    } else if(o){
      await query(`UPDATE orders SET payment_status='FAILED',failure_reason=$1 WHERE id=$2`,[`M-PESA payment cancelled/failed (ResultCode ${cb.ResultCode})`,o.id]);
      await audit('M-PESA',`STK payment failed for ${o.order_reference}`,'FAILED',{resultCode:cb.ResultCode,resultDesc:cb.ResultDesc});
    }
    res.json({ResultCode:0,ResultDesc:'Accepted'});
  }catch(e){console.error('stk callback',e);res.json({ResultCode:0,ResultDesc:'Accepted'});}
});
